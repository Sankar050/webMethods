...
<script type="text/javascript">
	...
	function populateDynamicValues(propertyKey, propertyValue) {
		$("select['"+ propertyKey+ "']").multiselect("select", propertyValue);
	}
		...
		%loop fields%
			...
			%ifvar isHidden equals('false')%
				...
				%ifvar isRequired equals('true')%
					<script>
						if("%value modelType%" === "Meta" && "%value isMultiSelect%" === "false") {
								...
								%ifvar lookupService -notempty%
									<script>
										if("%value propertyKey%".endsWith("x5t")) {
											document.write('<input id=input class="meta-field" name="META$%value propertyKey%" value="%value propertyValue%" size=60 /><input type="file" id="certFile" name="certFile" onchange="calculateX5T()" accept=".crt, .cer, .ca-bundle, .p7b, .p7c, .p7s, .pem" />');
										} else {
