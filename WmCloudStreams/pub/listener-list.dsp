<HTML>
    <head>
		...
		<SCRIPT LANGUAGE="JavaScript">
            ...
            // fetches runtime state reasons for the listener
            function getRuntimeStateReasonsForListener(element) {
                var listenerAlias = $.urlParam($(element).attr('href'), "alias");

                $.ajax({
                    type: 'GET',
                    url: '/invoke/wm.cloudstreams.listener:listRuntimeDetails?listenerAlias=' + listenerAlias,
                    dataType: 'json',
				...
				<TABLE class="tableView" WIDTH=100% id="head" name="head">
					<TR>
						<TD CLASS="heading" COLSPAN=7>
							<div style="float: left;">Listeners</div>
							<div align="right" style="padding-right:0.5em; cursor:pointer;"><img src="images/update.gif" title="Refresh all Listeners" border=0 onClick="location.reload(true);"></div>
						</TD>
					</TR>
                   ...
                   <script>writeTD('row-l');</script><span id="replaceWsWithNbsp">%value connectionTypeDisplayName%</span></td>
                    %switch state%
                        %case 'enabled'%
							...
							%ifvar listenerRuntimeState equals('ACTIVE')%
								<a class="imagelink" style="color:green;" href="/WmCloudStreams/connector-admin-nodes.dsp?listenerAction=&action=disable&listenerAlias=%value alias%&searchListenerName=%value ../searchListenerName%&providerName=%value /providerName%&groupName=%value /groupName%&listenerConnectionType=%value listenerConnectionType%&connectorID=%value /connectorID%&connectorName=%value /connectorName%&authenticationType=%value authenticationType%#tabs-2"
							ONCLICK="return confirmListenerDisable('%value alias%');">Enabled</a>
							%else%
								<a class="submodal" href="/WmCloudStreams/listener-runtime-state.dsp?alias=%value alias%" onmouseenter="javascript:getRuntimeStateReasonsForListener(this)"
									title="">Suspended</a>
							%endif%
                        %case 'disabled'%
								%invoke wm.cloudstreams.service.common:listRuntimeFields%
									%ifvar fields -notempty%
										...
										<script type="text/javascript" src="js/modal.js"></script>
										<a class="submodal" style="color:red;" href="/WmCloudStreams/listener-input-page.dsp?alias=%value alias%
										&providerName=%value /providerName%&groupName=%value /groupName%&connectorID=%value /connectorID%&connectorName=%value /connectorName%&authenticationType=%value authenticationType%&action=none">Disabled</a>
									%else%
										<script>writeTD('rowdata');</script>
										<a class="imagelink" style="color:red;" href="/WmCloudStreams/connector-admin-nodes.dsp?action=enable&listenerAlias=%value alias%&searchListenerName=%value ../searchListenerName%&providerName=%value /providerName%&groupName=%value /groupName%&listenerConnectionType=%value listenerConnectionType%&connectorID=%value /connectorID%&connectorName=%value /connectorName%&authenticationType=%value authenticationType%#tabs-2"
										ONCLICK="return confirmListenerEnable('%value alias%');">Disabled</a>
									%endif%
								%endinvoke%
                    %endswitch%
