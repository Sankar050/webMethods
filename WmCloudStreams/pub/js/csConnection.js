...
function populateRequiredMetaFields(fieldName)
{
  requiredMetaFields.push(fieldName);
}
...
function validateForm(theForm,forTest=false)
{
	...
	for(i = 0; i < requiredFields.length; i++) {
		if(!(theForm.elements[requiredFields[i]].name.includes('oauth_v20.authorizationServerUrl') 
			|| theForm.elements[requiredFields[i]].name.includes('oauth_v20.authenticationServerUrl'))) {
	...
	// check if session management is not none/auto and session expiry timeout value is required
	var timeoutTypeObj = theForm.elements["CMGRPROP$timeoutType"];
	var timeoutType = (!$(timeoutTypeObj).is(":disabled"))? timeoutTypeObj.options[timeoutTypeObj.selectedIndex].value : "none";
	var isTimeoutRequired = timeoutType == "idle" || timeoutType == "fixed";
	// connection pool is enablled?
	...
	for(i = 0; i < validationFields.length; i++) {
		...
		if($('[name$="pr.followServerRedirects"]')[0] != null) {
			var followServerRedirects = $('[name$="pr.followServerRedirects"]')[0].value;
			if(validationFields[i].endsWith('pr.serverRedirectMax') && followServerRedirects === 'true'
				&& (parseInt(theForm.elements[validationFields[i]].value) <= 0 || isNaN(parseInt(theForm.elements[validationFields[i]].value))) ) {
		...
		if(validationFields[i] === 'CMGRPROP$sessionExpiry' && poolable === 'true' &&  isTimeoutRequired
			&& (parseInt(theForm.elements[validationFields[i]].value) <= 0 || isNaN(parseInt(theForm.elements[validationFields[i]].value))) ) {
...
function setEnabledFields(theForm, val)
{
    ...
    setField(theForm.elements["CMGRPROP$timeoutType"],     val);
	val = CMGRPROP.timeoutType === "none" ? false : true;
    setField(theForm.elements["CMGRPROP$sessionExpiry"],     val);
}
...
function handlePoolableChange(theForm)
{
    ...
    if (poolable == "true") {
		...
		theForm.elements["CMGRPROP$timeoutType"].value = CMGRPROP.timeoutType;
		...
		CMGRPROP.timeoutType = theForm.elements["CMGRPROP$timeoutType"].value;
...
function getTokenField(tokenField){
	...
	function lookupFunction(element, targetElement, lookupService, form) {
		...
		dataFields["passmanHandles"] = passmanFieldsMap;
		dataFields["packageName"] = packageName;
