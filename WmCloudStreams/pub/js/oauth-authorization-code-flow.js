...
$(document).ready(function() {
	...
	$("input[name='passwordChange']").val("false");// clear passwordChange value.
...
function updateResponseFieldValues() {
	...
	// set client secret value only if it is changed
	console.debug("password changed fields='" + $('input[name="passwordChange"]').val() 
			+ "' includes oauth.consumerSecret? " + $('input[name="passwordChange"]').val().includes('oauth.consumerSecret'));
	if($('input[name="passwordChange"]').val().includes('oauth.consumerSecret')) {
	...
	// update meta fields' value in the connection page
	$.map(paramsMap, function( val, i ) {
		if(val['name'].startsWith("META")) {
			setParentFormFieldValue(getElementFullName(val['name']), val['value']);
		}
	});
...
function buildTableContent() {
	var clientIdLabel = $(getParentPageElementByName(connection_client_id_name)).closest('tr').children('td:first').text();
	var clientSecretLabel = $(getParentPageElementByName(connection_client_secret_name)).closest('tr').children('td:first').text();
    ...
    tableRows += "<tr><td>" + clientSecretLabel + "</td><td><input id=\"oauth.consumerSecret\" type=password size=60 name=\"oauth.consumerSecret\" onchange=\"passwordChanged(this.form, 'oauth.consumerSecret')\"></td></tr>";
    ...
    for (var i = 0; i < elements.length; i++) {
		...
		if(elementDisplayName.endsWith("*")) {
			requiredMetaFields.push(elementName);
		}
...
function constructAuthorizationUrl() {
    ...
    urlBuilder.push(authorizationServerUrl);
	urlBuilder.push("?response_type=code");
	urlBuilder.push("&client_id=" + client_id);
	if(getScope() != null) {
		urlBuilder.push("&scope=" + getScope());
	}
	urlBuilder.push("&redirect_uri=" + redirect_uri);
	urlBuilder.push("&state=" + uuid());
	console.debug(urlBuilder.join(""));
...
function getScope() {
	...
	if(!isUndefinedOrNULL(scopeElement)) {
		scopeValue = $(scopeElement).val();
	}
    return scopeValue;
}
...
function updateParametersValue() {
    ...
    pushFieldValue(paramsMap, "client_id", client_id, "oauth.consumerId");
    pushFieldValue(paramsMap, "client_secret", client_secret, "oauth.consumerSecret");

    for (var i = 0; i < paramsArray.length; i++) {
        var value = $("[name='" + paramsArray[i] + "']").val();
		if(isEmpty(value) && requiredMetaFields.includes(paramsArray[i]))
			missingFields.push(paramsArray[i]);
        else
	...
	console.debug("missingFields: " + missingFields);
}
